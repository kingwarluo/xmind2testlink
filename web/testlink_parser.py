...
def to_testlink_xml_content(testsuite):
    def build_suite_xml(parent_suite, parent_testsuite):
        for suite in parent_testsuite.sub_suites:
            ...
            # When the subSuites is null, terminate the recursion of testSuite
...
def build_testcase_xml(suite, suite_element):
    for testcase in suite.testcase_list:
        ...
        build_custom_fields_xml(testcase, testcase_element)
        build_keywords_xml(testcase.keywords, testcase_element)
...
def build_custom_fields_xml(testcase, testcase_element):
    if testcase.customFields:
        custom_fields_element = SubElement(testcase_element, Tags.custom_fields)

        for custom_field in testcase.customFields:
            assert isinstance(custom_field, CustomField)
            custom_field_element = SubElement(custom_fields_element, Tags.custom_field)
            build_text_field(custom_field_element, Tags.name, custom_field.name)
            build_text_field(custom_field_element, Tags.value, custom_field.value)
        custom_field_element = SubElement(custom_fields_element, Tags.custom_field)
        build_text_field(custom_field_element, Tags.name, "用例级别")
        if testcase.importance is None:
            testcase.importance = 2
        build_text_field(custom_field_element, Tags.value, str(testcase.importance))


def build_keywords_xml(keywords, testcase_element):
    if keywords:
        keywords_element = SubElement(testcase_element, Tags.keywords)

        for keyword in keywords:
            keyword_element = SubElement(keywords_element, Tags.keyword)
            keyword_element.set(Attributes.name, keyword)
            build_text_field(keywords_element, Tags.notes, keyword)