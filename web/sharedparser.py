...
def open_and_cache_xmind(xmind_file):
    if not cache:
        ...
        cache['custom'] = cache['sheet'][1]['topic']
...
def is_testsuite_topic(testsuite_dict):
    """ When makers is suite(suite_identity)， return True """
    if isinstance(testsuite_dict['makers'], list) \
            and testsuite_dict['makers'][0] == _config['suite_identity']:
        return True
    return False
